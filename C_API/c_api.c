...
int ISR_41_state(){
	char isr_41;
	FILE* fp_41 = fopen("/sys/kernel/ISR_41/isr_41_received", "r");
	fread(&isr_41, 1, 1, fp_41);
	fclose(fp_41);
	return isr_41 -'0';
...
int main(void)
{
          ...
          // loading interrupt files
          FILE* fp_40 = fopen("/sys/kernel/ISR_40/isr_40_received", "r");
          FILE* fp_41 = fopen("/sys/kernel/ISR_41/isr_41_received", "r");
          ...
          if(fp_40 == NULL || fp_41 ==  NULL){
        	  printf("\n one of Interupt Driversh has not been loaded! exiting..\n");
        	  return 0;
          }
          fclose(fp_40);
          fclose(fp_41);
		...
		for(frames =0; frames < 1; frames++){
	        ...
	        for(layer_ID = 0; layer_ID < No_of_Layers; layer_ID++){
			    ...
			    //printf("\nout_allocated_space_per_row: %d\n", out_allocated_space_per_row);
	...
	//		    }
			...
			// clearing last state interupt flag
			ISR_41_state();
		}
		clock_t end = clock();
